...
/*
 ...
 즉, Mapper 는 자바의 구성요소가 아니고 mybatis 의 구성요소이기 때문에 매칭하기 위해 Mapper 클래스가 있는 것
 => resources > mapper > AdminMapper 생성 / com.project.ecommerce > mapper > AdminMapper 도 생성
 생성하지 않았을 경우에는 오류가 발생한다.
 하지만 Mapper 는 Mybatis mapper 랑 연결해주는 역할이므로 실기능이 별로 없다. 따라서 class 가 아닌 interface 로 구현한다.
*/
@Mapper
public interface CategoryMapper {

/*
 insert, update, delete 는 리턴값이 없어 void 를 쓰지만 int 를 리턴받을 수도 있다.
...
*/
    ...
    * 하지만 DTO 로 사용하면 정의해주지 않아도 잘 인지한다.
    * */
